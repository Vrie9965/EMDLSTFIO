...
post_gif(){
        ...
        response="$(
                curl -sSLX POST \
                        ...
                        -w $'\n%{http_code}' \
                "${FRMENV_API_ORIGIN}/${FRMENV_FBAPI_VER}/${3}/comments?access_token=${FRMENV_FBTOKEN}"
        )" || {
                printf '%s\n' "[ERROR] post_gif: Network failure while posting GIF for frame ${1}" >&2
        ...
        if [[ "${response_status}" != "200" ]]; then
                local fb_err_msg fb_err_code
                fb_err_msg="$(jq -r '.error.message // empty' <<< "${response_body}" 2>/dev/null)"
                fb_err_code="$(jq -r '.error.code // empty' <<< "${response_body}" 2>/dev/null)"
                printf '%s\n' "[ERROR] post_gif: HTTP ${response_status:-unknown} code=${fb_err_code:-unknown} message=${fb_err_msg:-unknown}" >&2
                unset TEMP_CRAFTMESSAGE
                return 1
        fi
        unset TEMP_CRAFTMESSAGE response response_body response_status
}
...
post_randomcrop(){
        ...
        response="$(
                curl -sSLX POST \
                        ...
                        -w $'\n%{http_code}' \
                "${FRMENV_API_ORIGIN}/${FRMENV_FBAPI_VER}/${2}/comments?access_token=${FRMENV_FBTOKEN}"
        )" || {
                printf '%s\n' "[ERROR] post_randomcrop: Network failure while posting random crop for frame ${1}" >&2
                unset TEMP_CRAFTMESSAGE
                return 1
        }
        response_body="$(sed '$d' <<< "${response}")"
        ...
        if [[ "${response_status}" != "200" ]]; then
                ...
                fb_err_msg="$(jq -r '.error.message // empty' <<< "${response_body}" 2>/dev/null)"
                fb_err_code="$(jq -r '.error.code // empty' <<< "${response_body}" 2>/dev/null)"
                printf '%s\n' "[ERROR] post_randomcrop: HTTP ${response_status:-unknown} code=${fb_err_code:-unknown} message=${fb_err_msg:-unknown}" >&2
                unset TEMP_CRAFTMESSAGE
                return 1
        fi
        unset TEMP_CRAFTMESSAGE response response_body response_status
...
post_commentsubs(){
        ...
        if [[ -f "${commentsub_path}" ]]; then
                ...
                response="$(
                        ...
                        printf '%s\n' "[ERROR] post_commentsubs: Network failure for frame ${1}" >&2
                        return 1
                }
                response_body="$(sed '$d' <<< "${response}")"
                response_status="$(tail -n1 <<< "${response}")"
                if [[ "${response_status}" != "200" ]]; then
                        ...
                        fb_err_msg="$(jq -r '.error.message // empty' <<< "${response_body}" 2>/dev/null)"
                        fb_err_code="$(jq -r '.error.code // empty' <<< "${response_body}" 2>/dev/null)"
                        printf '%s\n' "[ERROR] post_commentsubs: HTTP ${response_status:-unknown} code=${fb_err_code:-unknown} message=${fb_err_msg:-unknown}" >&2
                        return 1
                fi
                return 0
        fi
...
post_album(){
        ...
        response="$(
                curl -sSLX POST \
                        ...
                        -F "message=${message}" \
                        -w $'\n%{http_code}' \
                "${FRMENV_API_ORIGIN}/${album}/photos?access_token=${FRMENV_FBTOKEN}&published=1"
        )" || {
                printf '%s\n' "[ERROR] post_album: Network failure for frame ${1} to album ${album}" >&2
                return 1
        }
        response_body="$(sed '$d' <<< "${response}")"
        response_status="$(tail -n1 <<< "${response}")"
        if [[ "${response_status}" != "200" ]]; then
                ...
                fb_err_msg="$(jq -r '.error.message // empty' <<< "${response_body}" 2>/dev/null)"
                fb_err_code="$(jq -r '.error.code // empty' <<< "${response_body}" 2>/dev/null)"
                printf '%s\n' "[ERROR] post_album: HTTP ${response_status:-unknown} code=${fb_err_code:-unknown} message=${fb_err_msg:-unknown}" >&2
                return 1
        fi
